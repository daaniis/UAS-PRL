...
class User29Controller extends Controller
{
    public function dashboardPage29()
    {
        ...
        $getUser = $client->request('GET', "{$API_URL}/user29")->getBody()->getContents();
    ...
    public function profilePage29()
    {
        ...
        $getUser = $client->request('GET', "{$API_URL}/user29/{$user->id}")->getBody()->getContents();
        $getAgama = $client->request('GET', "{$API_URL}/agama29")->getBody()->getContents();
    ...
    public function detailPage29(Request $request, $id)
    {
        ...
        $client = new Client();
        $getUser = $client->request('GET', "{$API_URL}/user29/{$id}")->getBody()->getContents();
        $user = json_decode($getUser, true)['data'];
    ...
    public function putUserDetail29(Request $request)
    {
        ...
        $putUser = $client->request('PUT', "{$API_URL}/user29/{$user->id}", [
            'json' => [
                'name' => $request->name,
                'email' => $request->email,
    ...
    public function putUserAgama29(Request $request, $id)
    {
        ...
        $putUserAgama = $client->request('PUT', "{$API_URL}/user29/{$id}/agama", [
            'json' => [
    ...
    public function putUserStatus29(Request $request, $id)
    {
        ...
        $putUserStatus = $client->request('PUT', "{$API_URL}/user29/{$id}/status", [
            'json' => [
    ...
    public function putUserPassword29(Request $request)
    {
        ...
        $putUserPassword = $client->request('PUT', "{$API_URL}/user29/{$user->id}/password", [
            'json' => [
    ...
    public function putUserPhoto29(Request $request)
    {
        ...
        $putUserPhoto = $client->request('PUT', "{$API_URL}/user29/{$id}/photo", [
            'headers' => [
    ...
    public function putUserPhotoKTP29(Request $request)
    {
        $API_URL = env('API_URL', "http://localhost/backend/uasbackend/public/api");
        ...
        $putUserPhoto = $client->request('PUT', "{$API_URL}/user29/{$id}/photoKTP", [
            'headers' => [
                'Accept' => 'application/json',
            ],
            'json' => [
    ...
    public function deleteUser29($id)
    {
        $API_URL = env('API_URL', "http://localhost/backend/uasbackend/public/api");

        $client = new Client();

        $deleteUser = $client->request('DELETE', "{$API_URL}/user29/{$id}", [
            'headers' => [
                'Accept' => 'application/json',
            ],
        ])->getBody()->getContents();
